...
Class LanguageController extends Controller {
    ...
    public function lists(Request $request) {
        ...
        foreach ($languages as $locale => $language) {
            ...
            $rows[] = array(
                ...
                '<a href="/language/' . $locale . '" class="md-trigger col s2"> <i class="mdi-action-visibility " style="font-size:20px" title="' . trans('messages.view') . '" data-toggle="tooltip"></i></a> ' .
                //'<a href="#" data-href="/language/'.$locale.'/edit" class="  col s2   md-trigger" data-toggle="modal" data-target="#myModal"> <i class="mdi-editor-border-color" title="'.trans('messages.edit').'" data-toggle="tooltip"></i></a> '.
                '<a href="/language/' . $locale . '/edit" class="col s2 " style="font-size:20px" data-toggle="modal"  > <i class="mdi-editor-mode-edit" data-toggle="tooltip" title="' . trans('messages.edit') . '"></i></a>' .
                delete_form(['language.destroy', $locale,'data-submit' => 'noAjax']) . '</div></div>',
                $locale,
                $language['language'],
    ...
    public function addWords(Request $request) {
        ...
        $language_translation = File::getRequire($filename);
        $language_translation[$data['key']] = $data['text'];
        File::put($filename, var_export($language_translation, true));
        File::prepend($filename, '<?php return ');
        File::append($filename, ';');
        ...
        if ($request->has('ajax_submit')) {
            $response = ['message' => trans('messages.word_or_sentence'), 'status' => 'success'];
            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
        }
