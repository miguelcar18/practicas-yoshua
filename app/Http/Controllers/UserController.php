...
class UserController extends Controller {
    ...
    public function lists(Request $request) {
        ...
        foreach ($users as $user) {
            $row = array(
                ...
                '<a href="/user/' . $user->id . '" class="col s1 mdi-action-visibility" style="font-size:20px"> <i class="fa fa-arrow-circle-o-right" data-toggle="tooltip" title="' . trans('messages.view') . '"></i></a>' .
                (($user->status == 'active' && Entrust::can('change-user-status')) ? '<a href="#" class="col s1 mdi-notification-event-available " style="font-size:20px" data-ajax="1" data-extra="&user_id=' . $user->id . '&status=ban" data-source="/change-user-status"> <i class="fa fa-ban" data-toggle="tooltip" title="' . trans('messages.ban') . ' ' . trans('messages.user') . '"></i></a>' : '') .
                (($user->status == 'banned' && Entrust::can('change-user-status')) ? '<a href="#" class="col s1 mdi-notification-event-busy" style="font-size:20px" data-ajax="1" data-extra="&user_id=' . $user->id . '&status=active" data-source="/change-user-status"> <i class="fa fa-check" data-toggle="tooltip" title="' . trans('messages.active') . ' ' . trans('messages.user') . '"></i></a>' : '') .
                (($user->status == 'pending_approval' && Entrust::can('change-user-status')) ? '<a href="#" class="mdi-notification-event-note" style="font-size:20px" data-ajax="1" data-extra="&user_id=' . $user->id . '&status=approve" data-source="/change-user-status"> <i class="fa fa-check" data-toggle="tooltip" title="' . trans('messages.approve') . ' ' . trans('messages.user') . '"></i></a>' : '') .
            ...
            elseif ($user->status == 'pending_activation')
                $status = '<span class="card deep-purple card-content white-text">' . trans('messages.pending') . ' ' . trans('messages.activation') . '</span>';
            elseif ($user->status == 'pending_approval')
                $status = '<span class="card green card-content white-text">' . trans('messages.pending') . ' ' . trans('messages.approval') . '</span>';
            elseif ($user->status == 'banned')
                $status = '<span class="card red card-content white-text">' . trans('messages.banned') . '</span>';
    ...
    public function profileUpdate(Request $request, $id) {
        ...
        $profile->date_of_birth = (Carbon::parse($request->input('date_of_birth'))->format('Y-m-d')) ? : null;
