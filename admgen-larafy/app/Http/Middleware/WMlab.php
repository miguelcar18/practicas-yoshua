...
class WMLab
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        ...
        if(isSecure())
            config(['config.allowed_upload_file' => 'jpg,png,doc,docx,pdf,txt']);

        $default_timezone = config('config.timezone_id') ? config('timezone.'.config('config.timezone_id')) : 'Asia/Kolkata';
        date_default_timezone_set($default_timezone);

        $default_language = (session()->has('lang')) ? session('lang') : ((config('config.default_language')) ? : 'en' );
        session()->put('lang', $default_language);
        ...
        $datepicker_language = (config('lang.'.$default_language.'.datepicker')) ? : 'en';
        $direction = (config('config.direction')) ? : 'ltr';
        view()->share(compact('direction','default_language','datatable_language','calendar_language','datepicker_language'));
